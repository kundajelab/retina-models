...
cp -r /home/users/surag/kundajelab/retina-models/src $BASE/models/$RUNNAME/fold0/code
...
for x in `ls $BASE/bigwigs`
 do
  ...
  sbatch --job-name train_$n \
         --output $BASE/models/$RUNNAME/fold0/logs/$n.log.txt \
         --error $BASE/models/$RUNNAME/fold0/logs/$n.err.txt \
         --mem 100G \
         $JOBSCRIPT train.py \
         ...
         --output-prefix $BASE/models/$RUNNAME/fold0/saved/$n \
