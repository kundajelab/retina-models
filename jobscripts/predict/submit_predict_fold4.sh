...
for x in `ls $BASE/bigwigs`
 do
  ...
  sbatch --job-name metrics_$n \
         --output $BASE/models/$RUNNAME/fold4/logs/$n.metrics.log.txt \
         --error $BASE/models/$RUNNAME/fold4/logs/$n.metrics.err.txt \
         --mem 50G \
         --time 1:00:00 \
         ...
         --output-prefix $BASE/models/$RUNNAME/fold4/metrics/$n \
         --model $BASE/models/$RUNNAME/fold4/saved/$n.h5 \
         --test-chr "chr7" "chr13" "chr17" "chr19" "chr21" "chrX"
 done
