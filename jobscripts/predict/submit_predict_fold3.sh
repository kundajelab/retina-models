...
for x in `ls $BASE/bigwigs`
 do
  ...
  sbatch --job-name metrics_$n \
         --output $BASE/models/$RUNNAME/fold3/logs/$n.metrics.log.txt \
         --error $BASE/models/$RUNNAME/fold3/logs/$n.metrics.err.txt \
         --mem 50G \
         --time 1:00:00 \
         ...
         --output-prefix $BASE/models/$RUNNAME/fold3/metrics/$n \
         --model $BASE/models/$RUNNAME/fold3/saved/$n.h5 \
         --test-chr "chr5" "chr10" "chr14" "chr18" "chr20" "chr22" 
 done
