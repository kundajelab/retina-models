...
for x in `ls $BASE/bigwigs`
 do
  ...
  sbatch --job-name metrics_$n \
         --output $BASE/models/$RUNNAME/fold1/logs/$n.metrics.log.txt \
         --error $BASE/models/$RUNNAME/fold1/logs/$n.metrics.err.txt \
         --mem 50G \
         --time 1:00:00 \
         ...
         --output-prefix $BASE/models/$RUNNAME/fold1/metrics/$n \
         --model $BASE/models/$RUNNAME/fold1/saved/$n.h5 \
         --test-chr "chr2" "chr8" "chr9" "chr16"
 done
